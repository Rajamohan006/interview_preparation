...
> [!NOTE]
> **Performance, Diagnostics & Low-End Device Optimization** has been extracted and expanded into a comprehensive 10-year-level reference guide:  
> 👉 [**`performance.md` — Android Performance on Low-End Devices**](./performance.md)
>
> Topics covered in depth inside [`performance.md`](./performance.md):
> * **Hardware Constraints:** 1 GB–3 GB RAM limits, slow eMMC flash storage, weak GPUs, thermal throttling.
> * **Memory Architecture:** ART Concurrent Copying GC, read barriers, LMKD & `oom_score_adj`, PSS vs RSS vs USS.
> * **Memory Leaks & Bitmaps:** GC Roots, LeakCanary mechanics, uncompressed ARGB_8888 downsampling, hardware bitmaps.
> * **CPU & Main Thread:** ANR thresholds (5s/10s/20s), reading `/data/anr/` traces, Choreographer VSYNC.
> * **UI Rendering:** Frame budgets (60Hz vs 90Hz vs 120Hz), JankStats, overdraw elimination, Compose recomposition skipping.
> * **Tooling Deep-Dive:** Android Studio Profiler vs. Perfetto (with PerfettoSQL) vs. Android GPU Inspector (AGI).
> * **Interview Preparation:** 25 Staff/Principal-level Q&As, 4 production scenarios, and the 9-step resolution framework.
>
> *(For app security, Keystore, ProGuard/R8, and cryptographic storage, see also [`testing_security.md`](./testing_security.md).)*
